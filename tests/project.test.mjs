...
test("five templates use distinct layouts and the spark music is beat-synced", async () => {
  ...
  assert.match(renderer, /function sampleSparkBackground/);
  assert.match(renderer, /function buildSparkBeatSlots/);
  assert.match(renderer, /function randomSparkInset/);
  assert.match(renderer, /pickSparkPhoto/);
  assert.match(renderer, /export const MAX_PHOTOS = 30/);
});
...
test("travel book turns beat-aligned pages and template typography is shared by preview and export", async () => {
  ...
  assert.match(renderer, /renderBook3D\(images, spreadIndex, width, height, progress, time, seed\)/);
  ...
  assert.match(bookRenderer, /createPageGeometry\("right", 56, 14\)/);
  assert.match(bookRenderer, /new THREE\.MeshStandardMaterial/);
  assert.doesNotMatch(bookRenderer, /color\.rgb \* vShade/);
  assert.match(bookRenderer, /float edgeLag = \(1\.0 - \$\{progressUniform\}\) \* \(1\.0 - pageU\)/);
  assert.match(bookRenderer, /float localAngle = turnArc \* \(1\.0 - edgeLag \* 0\.16\)/);
  assert.match(bookRenderer, /texture2D\(uBackMap, vec2\(1\.0 - vMapUv\.x/);
  assert.match(bookRenderer, /material\.forceSinglePass = true/);
  assert.match(bookRenderer, /function paintGridPaper/);
  assert.match(bookRenderer, /function drawFootnote/);
  assert.match(bookRenderer, /function drawTab/);
  assert.match(bookRenderer, /function drawTape/);
  assert.match(bookRenderer, /THREE\.PCFShadowMap/);
  assert.match(bookRenderer, /book\.leftMaterial\.map = current\.left/);
  assert.match(bookRenderer, /book\.flipPage\.visible = safeSpread < spreadCount - 1 && flip > 0\.001/);
  assert.match(bookRenderer, /preserveDrawingBuffer: true/);
  assert.match(renderer, /function drawTemplateTypography/);
  assert.match(renderer, /if \(String\(template\.id\) === "film"\) return/);
  assert.match(renderer, /textPreset: \{ title: "那年今日"/);
  assert.match(renderer, /subtitle: "快乐发生中"/);
  assert.match(renderer, /function drawSparkTypography/);
  assert.match(renderer, /function drawSparkCornerText/);
  assert.match(page, /封面标题/);
  assert.match(page, /中段短句/);
  assert.match(page, /结尾落款/);
  assert.match(page, /template\.id !== "film" && <div className="title-card">/);
